class LikesController < ApplicationController
  before_action :authenticate_user!

  def create
    question = Question.find params[:question_id]
    like = Like.new({user_id: current_user.id, question_id: question.id})

    if not can?(:like, question)
      flash[:danger] = "you are liking your own question"
      return redirect_to question_path(question)
    end

    if like.save
      redirect_to question_path(question), notice: "thanks for liking"
    else
      redirect_to question_path(question), notice: like.errors.full_messages.join(',')
    end
  end

  def destroy
    question = Question.find params[:question_id]
    like = Like.find params[:id]
    if like.destroy
      redirect_to question_path(question), notice: 'unliked'
    else
      redirect_to question_path(question), alert: like.errors.full_messages.join(',')
    end
  end
end
